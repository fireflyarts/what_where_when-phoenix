defmodule WhatWhereWhen.People.Person do
  ...
  def registration_changeset(person, attrs, opts \\ []) do
    person
    |> cast(attrs, [:id_name, :email, :password])
    |> validate_email()
  ...
  defp validate_email(changeset) do
    ...
    |> unsafe_validate_unique(:email, WhatWhereWhen.Repo)
  ...
  """
  def email_changeset(person, attrs) do
    person
    |> cast(attrs, [:email])
    |> validate_email()
    |> case do
  ...
  """
  def valid_password?(%WhatWhereWhen.People.Person{hashed_password: hashed_password}, password)
