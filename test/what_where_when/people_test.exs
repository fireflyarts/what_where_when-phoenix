defmodule WhatWhereWhen.PeopleTest do
  ...
  describe "register_person/1" do
    ...
    test "validates email and password when given" do
      {:error, changeset} = People.register_person(%{email: "not valid", password: "not valid"})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = People.register_person(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = People.register_person(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers people with a hashed password" do
      email = unique_person_email()
      {:ok, person} = People.register_person(valid_person_attributes(email: email))
  ...
  describe "change_person_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = People.change_person_registration(%Person{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        People.change_person_registration(
          %Person{},
  ...
  describe "apply_person_email/3" do
    ...
    test "requires email to change", %{person: person} do
      {:error, changeset} = People.apply_person_email(person, valid_person_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{person: person} do
      {:error, changeset} =
        People.apply_person_email(person, valid_person_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{person: person} do
      ...
      {:error, changeset} =
        People.apply_person_email(person, valid_person_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{person: person} do
      ...
      {:error, changeset} =
        People.apply_person_email(person, valid_person_password(), %{email: email})
    ...
    test "validates current password", %{person: person} do
      {:error, changeset} =
        People.apply_person_email(person, "invalid", %{email: unique_person_email()})
    ...
    test "applies the email without persisting it", %{person: person} do
      email = unique_person_email()
      {:ok, person} = People.apply_person_email(person, valid_person_password(), %{email: email})
      assert person.email == email
      assert People.get_person!(person.id).email != email
    end
  end
  ...
  describe "change_person_password/2" do
    ...
    test "allows fields to be set" do
      changeset =
        People.change_person_password(%Person{}, %{
  ...
  describe "update_person_password/3" do
    ...
    test "validates password", %{person: person} do
      {:error, changeset} =
        People.update_person_password(person, valid_person_password(), %{
          password: "not valid",
    ...
    test "validates current password", %{person: person} do
      {:error, changeset} =
        People.update_person_password(person, "invalid", %{password: valid_person_password()})
